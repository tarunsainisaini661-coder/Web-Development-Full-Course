...
1. The Magical sorting Hat: Imagine  you are a creating a magical sorting hat for a SIMT college.  Implemplement a Javascript function that takes an array of students names and assigns them to one of the four houses (Gryffindor (length less than 6),  Hufflepuff(length less than 8), Ravenclay(length less than 12), or Slytherin(length greater than or equal to 12)) based on the length of their names.
